...
NodoArbol * buscaNodoElemento(NodoArbol *raiz,int elemento){
    NodoArbol *recorrido;
    recorrido = raiz;
    while (recorrido) {
...
void recorrerEnOrden_Iterativo(ArbolBinarioBusqueda arbol){
    ...
    while (true) {
        ...
        cout << cima(pilaAux)<<"  ";
        recorrido = buscaNodoElemento(arbol.arbolBinario.raiz,desapilar(pilaAux));
...
void recorrerPreOrden_Iterativo(ArbolBinarioBusqueda arbol){
    ...
    while (not esPilaVacia(pilaAux)) {
        //muestra actual
        cout << cima(pilaAux) << endl;
        recorrido = buscaNodoElemento()
                //Por terminar 
    }
