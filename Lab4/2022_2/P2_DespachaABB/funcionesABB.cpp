...
void insertarRecursivo(struct NodoArbol *& raiz, NodoLote elemento) {
    if (esNodoVacio(raiz))
        ...
        if (raiz->elemento.lote > elemento.lote)
        insertarRecursivo(raiz->izquierda, elemento);
    else
        if (raiz->elemento.lote < elemento.lote)
        insertarRecursivo(raiz->derecha, elemento);
    else {
        //cout << "El elemento " << elemento.lote << "Ya se encuentra en el árbol" << endl;
...
bool buscarRecursivo(struct NodoArbol * nodo, int dato) {
    if (esNodoVacio(nodo))
        return false;
    if (comparaABB(nodo->elemento.lote, dato) == 0)
        return true;
    if (comparaABB(nodo->elemento.lote, dato) == 1)
...
struct NodoArbol * eliminarRecursivo(struct NodoArbol * nodo, int elemento) {
    ...
    if (nodo->elemento.lote > elemento)
        ...
        nodo->derecha = eliminarRecursivo(nodo->derecha, minNodo->elemento.lote);
    }
