...
void insertarAlInicio(struct Lista & tad, struct StrPedido elemento) {
...
void insertarAlFinal(struct Lista & tad, struct StrPedido elemento) {
...
struct Nodo * crearNodo(struct StrPedido elemento, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
...
//    }
//    
//    struct StrPedido pedido;
//    char buffId[5];
//    while (true) {
//        archText.getline(buffId,5,',');
//        if (archText.eof()) break;
//        strcpy(pedido.id,buffId);
//        archText >>
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento.id << " " << tiempoActual;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
